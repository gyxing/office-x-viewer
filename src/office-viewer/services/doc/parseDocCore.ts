...
async function blocksFromSegments(
  ...
  const builder = new DocBlockStreamBuilder({
    onBatch: options.onBatch
      ? ({ startIndex, blocks }) =>
          options.onBatch!(startIndex, blocks)
      : undefined,
  });
  let currentLine = '';
  ...
  const flushTable = async () => {
    ...
    if (rows.length === 1) {
      ...
      await builder.add(
        createTableBlock(rows, builder.nextSourceIndex),
      );
    }
  };
  ...
  const flushList = async () => {
    ...
    if (items.length === 1) {
      ...
      await builder.add(
        createListBlock(items, builder.nextSourceIndex),
      );
    }
  };
  ...
  for (let index = 0; index < normalizedSegments.length; index += 1) {
    ...
    if (segment.text === '\n') {
      ...
      const previousInline =
        currentLineInlines[currentLineInlines.length - 1];
    ...
    if ((index + 1) % 64 === 0) {
      await options.checkpoint({
        ...
        percent:
          0.7 +
          ((index + 1) /
            Math.max(1, normalizedSegments.length)) *
            0.22,
...
export async function parseDocCore(
  ...
  if (!isOleDoc(bytes)) {
    ...
    await context.output?.documentMetadata(
      documentMetadataFromDoc(document),
    );
